...
class Crudforgres:
    def __init__(self, user, password, host, port, dbname, schema,table, primarykey=None):
        self.user = user
    ...
    def _execute(self, query, Placeholder_value = "Nothing"):
        self._check_connection()
        if Placeholder_value == "Nothing":
            ...
            try:
                ...
                print(Placeholder_value +" not placed due to : " + error)
    ...
    def select_all(self, primaryKey_value = None):
        if primaryKey_value == None:
            select_query = sql.SQL("SELECT * FROM {}.{}").format(sql.Identifier(self.schema),sql.Identifier(self.table))
            self._execute( select_query )
        else:
            select_query = sql.SQL("SELECT * FROM {}.{} WHERE {} = {}").format(
                sql.Identifier(self.schema),
    ...
    def delete_all(self):
        delete_query  = sql.SQL("DELETE FROM {}.{}").format( sql.Identifier(self.schema),sql.Identifier(self.table) )
